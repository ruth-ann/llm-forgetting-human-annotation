...
def upload_to_github(local_path, github_path):
    ...
    resp_json = r_get.json()
    sha = resp_json.get("sha") if r_get.status_code == 200 and "sha" in resp_json else None
...
if st.session_state.phase == 1:
    ...
    if "phase1_seed" not in st.session_state:
        ...
        seed_file = save_seed_to_file(st.session_state.annotator, 1, st.session_state.phase1_seed)
...
# ----------------
    ...
    if "phase2_seed" not in st.session_state:
        ...
        seed_file = save_seed_to_file(st.session_state.annotator, 2, st.session_state.phase2_seed)
